...
&nbsp;
            if (!filename || processName.toLowerCase() === filenameLower) {
                var proc;
                <span class="missing-if-branch" title="if path not taken" >I</span>if (fullInfo) {
...
gpii.windows.isProcessRunning = function (proc) {
    var togo = false;
    <span class="missing-if-branch" title="else path not taken" >E</span>if (isNaN(proc)) {
        togo = gpii.windows.findProcessByName(proc) !== null;
    } else {
<span class="cstat-no" title="statement not covered" >        try {</span>
<span class="cstat-no" title="statement not covered" >            process.kill(proc, 0);</span>
<span class="cstat-no" title="statement not covered" >            togo = true;</span>
        } catch (e) {
<span class="cstat-no" title="statement not covered" >            togo = false;</span>
        }
    }
    return togo;
};
...
&nbsp;
    var waitOptions = {
        ...
        error: {
            ...
            " to " + (options.start ? <span class="branch-0 cbranch-no" title="branch not covered" >"start" </span>: "terminate") + " after " + options.timeout + "ms"
        }
    };
&nbsp;
 ...
 */
gpii.windows.waitForProcessStart = <span class="fstat-no" title="function not covered" >fu</span>nction (proc, userOptions) {
    var options = <span class="cstat-no" title="statement not covered" >fluid.extend(true, {start: true}, userOptions);</span>
<span class="cstat-no" title="statement not covered" >    return gpii.windows.waitForProcessState(proc, options);</span>
};
&nbsp;
/**
 ...
 * @return {promise}
 */
gpii.windows.closeProcessByName = <span class="fstat-no" title="function not covered" >fu</span>nction (filename, options) {
    var defaultOptions = <span class="cstat-no" title="statement not covered" >{</span>
        timeout: 15000,
        cleanOnly: false,
        exitCode: 0
    };
&nbsp;
<span class="cstat-no" title="statement not covered" >    options = fluid.extend(defaultOptions, options);</span>
&nbsp;
    var pids = <span class="cstat-no" title="statement not covered" >gpii.windows.findProcessByName(filename, true);</span>
<span class="cstat-no" title="statement not covered" >    if (!pids) {</span>
        // Process is not running.
<span class="cstat-no" title="statement not covered" >        return fluid.toPromise(true);</span>
    }
&nbsp;
    var foundWindow = <span class="cstat-no" title="statement not covered" >false;</span>
&nbsp;
    // Enumerate all the top-level windows on the desktop, to see which ones are owned by the process.
<span class="cstat-no" title="statement not covered" >    gpii.windows.enumerateWindows(<span class="fstat-no" title="function not covered" >fu</span>nction (hwnd) {</span>
        // Get the process ID that owns the Window.
        var ptr = <span class="cstat-no" title="statement not covered" >ref.alloc(windows.types.DWORD);</span>
<span class="cstat-no" title="statement not covered" >        gpii.windows.user32.GetWindowThreadProcessId(hwnd, ptr);</span>
        var windowPid = <span class="cstat-no" title="statement not covered" >ptr.deref();</span>
&nbsp;
<span class="cstat-no" title="statement not covered" >        if (pids.indexOf(windowPid) !== -1) {</span>
            // Send WM_QUIT, which tells the thread to terminate.
<span class="cstat-no" title="statement not covered" >            gpii.windows.user32.PostMessageW(hwnd, windows.API_constants.WM_QUIT, options.exitCode, 0);</span>
<span class="cstat-no" title="statement not covered" >            foundWindow = true;</span>
        }
    });
&nbsp;
    var promiseTogo = <span class="cstat-no" title="statement not covered" >fluid.promise();</span>
&nbsp;
<span class="cstat-no" title="statement not covered" >    if (foundWindow) {</span>
        // Wait for the process to die.
<span class="cstat-no" title="statement not covered" >        gpii.windows.waitForProcessTermination(filename, { timeout: options.timeout })</span>
            .then(<span class="fstat-no" title="function not covered" >fu</span>nction () {
<span class="cstat-no" title="statement not covered" >                promiseTogo.resolve(true);</span>
...
gpii.windows.getProcessPath = function (pid, processHandle) {
    ...
    var togo = null;
&nbsp;
    <span class="missing-if-branch" title="if path not taken" >I</span>if (processHandle) {
<span class="cstat-no" title="statement not covered" >        hProcess = processHandle;</span>
...
&nbsp;
            ...
            <span class="missing-if-branch" title="else path not taken" >E</span>if (hProcess !== processHandle) {
