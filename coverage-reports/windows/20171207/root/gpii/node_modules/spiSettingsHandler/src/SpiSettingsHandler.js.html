  ...
  <div class='pad1'>
    ...
    <div class='clearfix'>
      <div class='fl pad1y space-right2'>
        ...
        <span class='fraction'>119/157</span>
      </div>
      <div class='fl pad1y space-right2'>
...
<span class="branch-0 cbranch-no" title="branch not covered" >        gpii.windows.bufferToArray(pvParam, payload.options.pvParam.valueType) </span>:
        pvParam.deref();
&nbsp;
 ...
 */
gpii.windows.spi.callProblematicSpi = <span class="fstat-no" title="function not covered" >fu</span>nction (pvParamType, action, uiParam, pvParam) {
    var cp = <span class="cstat-no" title="statement not covered" >require("child_process");</span>
&nbsp;
    var primitiveType = <span class="cstat-no" title="statement not covered" >pvParamType in gpii.windows.types;</span>
&nbsp;
...
&nbsp;
    var child = <span class="cstat-no" title="statement not covered" >cp.fork(__dirname + "/SpiChildProcess.js", options);</span>
    var promise = <span class="cstat-no" title="statement not covered" >fluid.promise();</span>
&nbsp;
    var timer = <span class="cstat-no" title="statement not covered" >setTimeout(<span class="fstat-no" title="function not covered" >fu</span>nction () {</span>
...
&nbsp;
<span class="cstat-no" title="statement not covered" >    child.on("exit", <span class="fstat-no" title="function not covered" >fu</span>nction (code) {</span>
...
<span class="cstat-no" title="statement not covered" >            promise.resolve(code);</span>
        }
    });
&nbsp;
...
&nbsp;
    ...
    gpii.windows.spi.waitForSpi()
        .then(function () {
&nbsp;
            <span class="missing-if-branch" title="if path not taken" >I</span>if (problematicSpiCalls.indexOf(action) &gt;= 0) {
                var p = <span class="cstat-no" title="statement not covered" >gpii.windows.spi.callProblematicSpi(pvParamType, action, uiParam, pvParam);</span>
<span class="cstat-no" title="statement not covered" >                fluid.promise.follow(p,  promiseTogo);</span>
            } else {
...
&nbsp;
    <span class="missing-if-branch" title="else path not taken" >E</span>if (uiParam === "struct_size") {
        ...
        <span class="missing-if-branch" title="if path not taken" >I</span>if (payload.options.pvParam.name === "NONCLIENTMETRICS" &amp;&amp; <span class="branch-1 cbranch-no" title="branch not covered" >os.release() &lt; "6")</span> {
<span class="cstat-no" title="statement not covered" >            result -= 4; </span>// do not include NONCLIENTMETRICS.iPaddedBorderWidth
        }
        return result;
    }
&nbsp;
...
<span class="branch-1 cbranch-no" title="branch not covered" >    case "array":</span>
        var length = <span class="cstat-no" title="statement not covered" >payload.options.pvParam.length;</span>
        var size = <span class="cstat-no" title="statement not covered" >ref.types[gpii.windows.types[payload.options.pvParam.valueType]].size;</span>
<span class="cstat-no" title="statement not covered" >        pvParam = new Buffer(length * size);</span>
...
&nbsp;
    <span class="missing-if-branch" title="if path not taken" >I</span>if (payload.options.pvParam.type === "array") {
<span class="cstat-no" title="statement not covered" >        pvParam = gpii.windows.arrayToBuffer(pvParam, payload.options.pvParam.valueType);</span>
...
gpii.windows.spi.GPII1873_HighContrastBug = function (method, payload, results) {
    if ((payload.options.pvParam.type === "struct") &amp;&amp;
        ...
        <span class="missing-if-branch" title="if path not taken" >I</span>if (payload.settings.HighContrastTheme &amp;&amp;
<span class="branch-1 cbranch-no" title="branch not covered" >            payload.settings.HighContrastTheme.value === "")</span> {
<span class="cstat-no" title="statement not covered" >            fluid.log("Bug #1873 detected. We are forcing the return value to empty string although the system cannot set to it.");</span>
<span class="cstat-no" title="statement not covered" >            if (method === "SET") {</span>
<span class="cstat-no" title="statement not covered" >                results.HighContrastTheme.newValue = "";</span>
            } else {
<span class="cstat-no" title="statement not covered" >                results.HighContrastTheme.value = "";</span>
            }
        }
    }
};
