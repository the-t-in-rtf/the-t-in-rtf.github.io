  ...
  <div class='pad1'>
    ...
    <div class='clearfix'>
      <div class='fl pad1y space-right2'>
        ...
        <span class='fraction'>144/168</span>
      </div>
      <div class='fl pad1y space-right2'>
      ...
      <div class='fl pad1y space-right2'>
        ...
        <span class='fraction'>143/167</span>
      </div>
    </div>
  </div>
...
&nbsp;
     ...
     */
    gpii.matchMakerFramework.utils.disposeSolutions = <span class="fstat-no" title="function not covered" >function (data, strategy, transformSpec, ontologyMetadata) {</span>
<span class="cstat-no" title="statement not covered" >        var solrecs = gpii.matchMakerFramework.utils.expandSolutions(data.solutionsRegistryEntries, transformSpec);</span>
<span class="cstat-no" title="statement not covered" >        var togo = {};</span>
        // calculate match for each of the contexts
<span class="cstat-no" title="statement not covered" >        fluid.each(data.hierarchicalPrefs.contexts, <span class="fstat-no" title="function not covered" >function (context, contextId) {</span></span>
<span class="cstat-no" title="statement not covered" >            var tmpSolrecs = fluid.copy(solrecs);</span>
...
&nbsp;
<span class="cstat-no" title="statement not covered" >            var leaves = gpii.matchMakerFramework.utils.computeLeaves(context.preferences);</span>
<span class="cstat-no" title="statement not covered" >            var disposed = strategy(leaves, tmpSolrecs, data, ontologyMetadata);</span>
<span class="cstat-no" title="statement not covered" >            togo[contextId] = [];</span>
<span class="cstat-no" title="statement not covered" >            fluid.each(disposed, <span class="fstat-no" title="function not covered" >function (solrec, solid) {</span></span>
...
&nbsp;
            ...
            for (var solutionId in sols) {
                ...
                for (var otherId in sols) {
                    ...
                    <span class="missing-if-branch" title="else path not taken" >E</span>if (gpii.matchMakerFramework.utils.containsSameValue(solutionType, typeInfo.solutionTypes[otherId])) {
...
&nbsp;
     ...
     */
    gpii.matchMakerFramework.utils.parsePriorities = <span class="fstat-no" title="function not covered" >function (context, solrecs) {</span>
        // for each entry in the metadata block:
<span class="cstat-no" title="statement not covered" >        fluid.each(context.metadata, <span class="fstat-no" title="function not covered" >function (metadata) {</span></span>
<span class="cstat-no" title="statement not covered" >            if (metadata.type === "priority") {</span>
<span class="cstat-no" title="statement not covered" >                fluid.each(metadata.scope, <span class="fstat-no" title="function not covered" >function (applicationId) {</span></span>
                    // set the priority of the application if it's part of our solrecs list:
<span class="cstat-no" title="statement not covered" >                    var solrec = solrecs[applicationId.substr("http://registry.gpii.net/applications/".length)];</span>
...
&nbsp;
    gpii.matchMakerFramework.utils.addPriorityFromApplicationSettings = <span class="fstat-no" title="function not covered" >function (preferences, solrecs) {</span>
<span class="cstat-no" title="statement not covered" >        fluid.each(preferences.applications, <span class="fstat-no" title="function not covered" >function (value, applicationId) {</span></span>
<span class="cstat-no" title="statement not covered" >            var sol = solrecs[applicationId];</span>
...
&nbsp;
        return fluid.remove_if(fluid.copy(allPreferences), function (value, preference) {
            ...
            return !(preference.startsWith("http://registry.gpii.net/common/") || <span class="branch-1 cbranch-no" title="branch not covered" >preference.startsWith(applicationString))</span>;
        });
    };
&nbsp;
    /**
...
&nbsp;
            ...
            <span class="missing-if-branch" title="if path not taken" >I</span>if (fullModel.preferences.contexts[contextId].conditions) {
