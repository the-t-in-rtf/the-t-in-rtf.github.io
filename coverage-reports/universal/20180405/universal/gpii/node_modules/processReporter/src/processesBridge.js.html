...
gpii.processes.findSolutionsByPids = function (that, pids) {
    return fluid.accumulate(pids, function (aPid, matches) {
        ...
        <span class="missing-if-branch" title="else path not taken" >E</span>if (found !== null) {
            matches.push(found);
        }
        return matches;
    }, []);
...
gpii.processes.findProcessesByCommand = function (that, commandName, procArray) {
    <span class="missing-if-branch" title="else path not taken" >E</span>if (!procArray) {
...
gpii.processes.findFirstProcessByCommand = function (that, commandName, procArray) {
    ...
    <span class="missing-if-branch" title="else path not taken" >E</span>if (commands.length &gt; 0) {
        return commands[0];
    }
    else {
...
gpii.processes.updateProcInfo = function (that, procInfo) {
    ...
    <span class="missing-if-branch" title="if path not taken" >I</span>if (procInfo.state === "NoSuchProcess") {
