...
&nbsp;
    gpii.indexedDynamicComponent.onCreate = <span class="fstat-no" title="function not covered" >fu</span>nction (that) {
        var key = <span class="cstat-no" title="statement not covered" >fluid.getForComponent(that, that.options.dynamicIndexKeyPath);</span>
        var ourPath = <span class="cstat-no" title="statement not covered" >fluid.pathForComponent(that);</span>
        var memberName = <span class="cstat-no" title="statement not covered" >ourPath[ourPath.length - 1];</span>
        var index = <span class="cstat-no" title="statement not covered" >fluid.get(that.dynamicIndexTarget, that.options.dynamicIndexTargetPath);</span>
<span class="cstat-no" title="statement not covered" >        index[key] = memberName;</span>
    };
&nbsp;
    gpii.indexedDynamicComponent.onDestroy = <span class="fstat-no" title="function not covered" >fu</span>nction (that) {
        var key = <span class="cstat-no" title="statement not covered" >fluid.getForComponent(that, that.options.dynamicIndexKeyPath);</span>
        var index = <span class="cstat-no" title="statement not covered" >fluid.get(that.dynamicIndexTarget, that.options.dynamicIndexTargetPath);</span>
<span class="cstat-no" title="statement not covered" >        if (index) { // workaround for FLUID-5930</span>
<span class="cstat-no" title="statement not covered" >            delete index[key];</span>
        }
    };
&nbsp;
})();
