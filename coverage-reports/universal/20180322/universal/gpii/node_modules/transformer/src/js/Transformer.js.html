  ...
  <div class='pad1'>
    ...
    <div class='clearfix'>
      <div class='fl pad1y space-right2'>
        ...
        <span class='fraction'>13/52</span>
      </div>
      <div class='fl pad1y space-right2'>
...
(function () {
     ...
     */
    gpii.transformer.transformOneSettingsHandler = <span class="fstat-no" title="function not covered" >fu</span>nction (settingsHandler, oneUserSolution, solutionId) {
        // TODO GPII-1223: throw app specific settings in the correct locations if there are multiple handlers
        var settings = <span class="cstat-no" title="statement not covered" >fluid.copy(oneUserSolution.settings["http://registry.gpii.net/applications/" + solutionId]);</span>
        // extract any common terms that were defined in the application block and delete them from the settings block
        var scopedCommon = <span class="cstat-no" title="statement not covered" >{};</span>
<span class="cstat-no" title="statement not covered" >        fluid.each(settings, <span class="fstat-no" title="function not covered" >fu</span>nction (val, key) {</span>
<span class="cstat-no" title="statement not covered" >            if (key.startsWith("http://registry.gpii.net/common/")) {</span>
<span class="cstat-no" title="statement not covered" >                scopedCommon[key] = fluid.copy(val); </span>// add to list of scoped common terms
<span class="cstat-no" title="statement not covered" >                delete settings[key]; </span>// remove from the settings
            }
        });
&nbsp;
<span class="cstat-no" title="statement not covered" >        if (settingsHandler.capabilitiesTransformations) {</span>
            var inferred = <span class="cstat-no" title="statement not covered" >fluid.model.transformWithRules(oneUserSolution.settings, settingsHandler.capabilitiesTransformations);</span>
            var inferredScoped = <span class="cstat-no" title="statement not covered" >(scopedCommon === undefined || $.isEmptyObject(scopedCommon)) ? {} : fluid.model.transformWithRules(scopedCommon, settingsHandler.capabilitiesTransformations);</span>
<span class="cstat-no" title="statement not covered" >            settings = fluid.extend(true, {}, inferred, inferredScoped, settings);</span>
        }
&nbsp;
<span class="cstat-no" title="statement not covered" >        settingsHandler.settings = settings;</span>
<span class="cstat-no" title="statement not covered" >        delete settingsHandler.capabilitiesTransformations;</span>
<span class="cstat-no" title="statement not covered" >        delete settingsHandler.capabilities;</span>
<span class="cstat-no" title="statement not covered" >        return settingsHandler;</span>
    };
&nbsp;
    /**
     ...
     */
    gpii.transformer.configurationToSettings = <span class="fstat-no" title="function not covered" >fu</span>nction (configuration, solutionsRegistryEntries) {
        var togo = <span class="cstat-no" title="statement not covered" >fluid.transform(configuration.applications, <span class="fstat-no" title="function not covered" >fu</span>nction (oneUserSolution, solutionId) {</span>
            var oneSolution = <span class="cstat-no" title="statement not covered" >fluid.copy(solutionsRegistryEntries[solutionId]);</span>
<span class="cstat-no" title="statement not covered" >            if (oneSolution === undefined) {</span>
...
<span class="cstat-no" title="statement not covered" >            delete oneSolution.contexts; </span>// solutions registry entries should include this as "old device reporter context info"
<span class="cstat-no" title="statement not covered" >            oneSolution.settingsHandlers = fluid.transform(oneSolution.settingsHandlers, <span class="fstat-no" title="function not covered" >fu</span>nction (settingsHandler) {</span>
<span class="cstat-no" title="statement not covered" >                return gpii.transformer.transformOneSettingsHandler(settingsHandler, oneUserSolution, solutionId);</span>
            });
<span class="cstat-no" title="statement not covered" >            oneSolution.active = oneUserSolution.active; </span>// copy over active directive for later use
<span class="cstat-no" title="statement not covered" >            return oneSolution;</span>
        });
&nbsp;
        // ensure no empty entries exist in the settings payload
<span class="cstat-no" title="statement not covered" >        for (var i in togo) {</span>
<span class="cstat-no" title="statement not covered" >            if (togo[i] === undefined) {</span>
...
&nbsp;
    gpii.transformer.booleanToNumber = <span class="fstat-no" title="function not covered" >fu</span>nction (value) {
